...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(const CellLook& cell_look) const = 0;
    ...
    public:
        ...
        double Evaluate(const CellLook& cell_look) const override {
            return cell_look(*cell_ptr_);
        }
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const CellLook& cell_look) const override {
                if (type_ == Type::Add)
                    return lhs_->Evaluate(cell_look) + rhs_->Evaluate(cell_look);
                else if (type_ == Type::Divide) {
                    if (rhs_->Evaluate(cell_look) == 0)
        ...
        public:
            ...
            double Evaluate(const CellLook& cell_look) const override {
                if (type_ == Type::UnaryMinus)
                    return -operand_->Evaluate(cell_look);
...
double FormulaAST::Execute(const CellLook& cell_look) const {
    return root_expr_->Evaluate(cell_look);
}
